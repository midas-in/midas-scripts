...
def validate_dicom_file(dicom_file_path):
    try:
        ...
        if result.returncode == 0:
            ...
            logger.error(f"Validation failed for {dicom_file_path} with error: {error_output}")
    except Exception as e:
        logger.exception(f"Error during DICOM validation: {e}")

def jpeg_to_dicom(jpeg_path, dicom_output_path, patient_name, patient_id, description, modality, accessionNumber, visit_date, series_uid, study_uid, body_part, series_description):
    ...
    try:
        ...
        ds.Modality = modality  # Secondary Capture
        ...
        # ds.StudyDate = visit_date.strftime('%Y%m%d')  # Set study date
        # ds.StudyTime = dt.strftime('%H%M%S')  # Set study time
        ...
        ds.SeriesNumber = "1"  # Example series number
        ...
        logger.info(f"Saved DICOM file: {output_file}")
        validate_dicom_file(output_file)
    except Exception as e:
        logger.exception(f"Failed to convert and save DICOM for {jpeg_path}: {e}")



def convert_folder(jpeg_folder, output_folder):
    os.makedirs(output_folder, exist_ok=True)
    logger.info(f"Converting folder: {jpeg_folder}")
    ...
    accessionNumber = os.path.basename(jpeg_folder)
    if 'XC' in jpeg_folder:
        description = "Clinical Image"
        modality= "XC"
        series_description = "External-camera Photography"
    elif 'RG' in jpeg_folder:
        description = "Radiography"
        modality= "RG"
        series_description = "Radiographic imaging"
    elif 'OT' in jpeg_folder:
        description = "Gross"
        modality= "OT"
        series_description = "Other Modality"

    for filename in os.listdir(jpeg_folder):
        ## Generate a unique SeriesInstanceUID for this series # Generate a single StudyInstanceUID for the entire study
        if filename.lower().endswith((".jpg", ".jpeg", ".png", ".tif", ".tiff")):
            jpeg_path = os.path.join(jpeg_folder, filename)
            jpeg_to_dicom(jpeg_path, output_folder, patient_name, patient_name, description, modality, accessionNumber, visit_date, series_uid, study_uid, body_part, series_description)

if __name__ == "__main__":
    if len(sys.argv) != 3:
        logger.error("Usage: python main.py <input_folder> <output_folder>")
        sys.exit(1)
